"""
Tests for Requirements Expert

Run with: pytest tests/test_expert.py -v
Or via: ./scripts/test-expert.sh requirements
"""
...
class TestKnowledgeFiles:
    ...
    def test_core_knowledge_exists(self):
        ...
        md_files = list(core_path.glob("*.md"))
        assert len(md_files) >= 2, "Expected question-patterns.md and behavior-contract-format.md"
    ...
    def test_template_exists(self):
        """Behavior Contract template must exist."""
        template_path = EXPERT_ROOT / "templates" / "behavior-contract.md"
        assert template_path.exists(), "templates/behavior-contract.md not found"
...
class TestConfiguration:
    ...
    def test_expert_name_set(self, config):
        ...
        assert expert_name == "requirements", f"Expected 'requirements', got '{expert_name}'"
    ...
    def test_port_configured(self, config):
...
class TestBoundaries:
    ...
    def test_refuse_patterns_configured(self, config):
        """Refuse patterns should include implementation-related terms."""
        boundaries = config.get("boundaries", {})
        refuse_patterns = boundaries.get("refuse_patterns", [])
        assert len(refuse_patterns) > 0, "No refuse patterns configured"
        # Check for implementation-related patterns
        patterns_str = " ".join(refuse_patterns).lower()
        assert "implement" in patterns_str or "code" in patterns_str, (
            "Refuse patterns should include implementation-related terms"
        )
...
class TestReadme:
    ...
    def test_readme_describes_domain(self):
        """README must describe the requirements domain."""
        readme_path = EXPERT_ROOT / "README.md"
        content = readme_path.read_text().lower()
        assert "behavior contract" in content, "README should mention Behavior Contracts"
        assert "requirements" in content, "README should mention requirements"


class TestRequirementsSpecific:
    """Tests specific to the Requirements expert."""

    def test_question_patterns_exist(self):
        """Question patterns knowledge file must exist."""
        patterns_path = EXPERT_ROOT / "knowledge" / "core" / "question-patterns.md"
        assert patterns_path.exists(), "question-patterns.md not found"

    def test_contract_format_exists(self):
        """Behavior Contract format knowledge file must exist."""
        format_path = EXPERT_ROOT / "knowledge" / "core" / "behavior-contract-format.md"
        assert format_path.exists(), "behavior-contract-format.md not found"

    def test_skill_covers_key_scenarios(self):
        """SKILL.md should cover key requirement scenarios."""
        skill_path = EXPERT_ROOT / "SKILL.md"
        ...
        for concept in key_concepts:
            assert concept in content, f"SKILL.md should mention '{concept}'"
...
class TestIntegration:
    ...
    @pytest.mark.integration
    def test_execute_generates_contract(self):
        """Execute tool should generate a Behavior Contract."""
        pytest.skip("Integration test - run with live server")
